...
body.addEventListener("click", (eve)=>{
    for (const r of allRows)if (r.classList.contains("active")) r.classList.remove("active");
...
function cellValue(row, columnIndex) {
    ...
    if (string.length !== 0 && !Number.isNaN(Number(string))) return Number(string);
...
function sortRows(rows, columnIndex, direction) {
    rows.sort((a, b)=>{
        ...
        let result;
        if (!isNaN(valA) && !isNaN(valB)) result = valA - valB;
...
headers.forEach((header)=>{
    header.addEventListener("click", ()=>{
        ...
        if (currentIndex !== lastIndex) clickCount = 0;
        clickCount++;
...
function pushNotification(type) {
    ...
    if (type === "warning") {
        ...
        messageDescription.textContent = "Please verify the information before finalizing.";
    }
    if (type === "success") {
        messageTitle.textContent = "Employee Added Successfully";
        message.classList.add("success");
        messageDescription.textContent = "The new employee record has been created and saved to the database.";
    }
    if (type === "error") {
        messageTitle.textContent = "Failed to Add Employee";
        message.classList.add("error");
        messageDescription.textContent = "Please ensure all required fields are filled correctly.";
    }
...
button.addEventListener("click", ()=>{
    ...
    for (const input of inputs)if (input.value.length === 0) {
